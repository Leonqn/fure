...
impl<T, E> Attempts<T, E> {
    ...
    pub fn new(max_retries: usize) -> Self {
        Self {
            max_retries,
            retry_if: |result| !matches!(result, Ok(_)),
        }
    }

    /// Create a policy which retries if `retry_if` returns true and retries limit `max_retries` isn't exceeded.
    pub fn with_retry_if(max_retries: usize, retry_if: fn(Result<&T, &E>) -> bool) -> Self {
        Self {
            max_retries,
            retry_if,
        }
